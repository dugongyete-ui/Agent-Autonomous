...
class Provider:
    def __init__(self, provider_name, model, server_address="127.0.0.1:5000", is_local=False):
        ...
        self.unsafe_providers = ["openai", "groq", "deepseek", "dsk_deepseek", "together", "google", "openrouter", "huggingface"]
    ...
    def get_api_key(self, provider):
        ...
        if not api_key:
            pretty_print(f"API key {api_key_var} not found. Please set it as an environment variable/secret.", color="warning")
            raise ValueError(f"API key {api_key_var} not found. Please set it as an environment variable.")
        return api_key
    ...
    def groq_fn(self, history, verbose=False):
        """
        Use Groq API to generate text. Groq uses OpenAI-compatible API.
        """
